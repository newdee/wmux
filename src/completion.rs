...
# values of `-L` (sockets) and `completion`.
...
{value_table}
  ...
  $setters = @('set','setw','set-option','set-window-option','show','showw','show-options','show-window-options')
  ...
  if ($rest.Count -eq 0) {{
    ...
    $out = @('powershell')
  }} elseif ($rest[0] -in $setters -and $prev -ne '-t' -and $wordToComplete -notlike '-*') {{
