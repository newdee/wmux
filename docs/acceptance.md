...
## 62. 网站章节按 README 的叙事重排

01 在 pane 之间派活 → 02 关掉终端程序照跑 → 03 命令时间和历史 → 04 后台任务提醒 → 05 分屏 → 06 全屏/列表/菜单 → 07 手机 → 08 配置 → 09 状态栏 → 10 安装（原来派活在第 07 章）。首屏标签去掉"原生 Windows"（跨平台打算）；下载按钮与系统要求照实保留。测试数 294 → 297。
